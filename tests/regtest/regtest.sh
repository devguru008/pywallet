...
alice="./run_pywallet --regtest -D /tmp/alice"
bob="./run_pywallet --regtest -D /tmp/bob"
carol="./run_pywallet --regtest -D /tmp/carol"
...
function wait_until_htlcs_settled()
{
    ...
    cmd="./run_pywallet --regtest -D /tmp/$1"
...
if [[ $1 == "init" ]]; then
    echo "initializing $2"
    rm -rf /tmp/$2/
    agent="./run_pywallet --regtest -D /tmp/$2"
...
if [[ $1 == "unixsockets" ]]; then
    ...
    ./run_pywallet --regtest daemon -d --rpcsock=unix # Start daemon with unix domain socket
    ./run_pywallet --regtest stop # Errors if it can't connect
    # Test custom socket path
    f=$(mktemp --dry-run)
    ./run_pywallet --regtest daemon -d --rpcsock=unix --rpcsockpath=$f
    [ -S $f ] # filename exists and is socket
    ./run_pywallet --regtest stop
    rm $f # clean up
    # Test for regressions in the ordinary TCP functionality.
    ./run_pywallet --regtest daemon -d --rpcsock=tcp
    ./run_pywallet --regtest stop
fi
