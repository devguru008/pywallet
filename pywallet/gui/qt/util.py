...
from pywallet.util import FileImportFailed, FileExportFailed, resource_path
from pywallet.util import EventListener, event_listener, get_logger
from pywallet.invoices import PR_UNPAID, PR_PAID, PR_EXPIRED, PR_INFLIGHT, PR_UNKNOWN, PR_FAILED, PR_ROUTING, PR_UNCONFIRMED, PR_BROADCASTING, PR_BROADCAST
from pywallet.logging import Logger
from pywallet.qrreader import MissingQrDetectionLib
...
def import_meta_gui(pywallet_window: 'PywalletWindow', title, importer, on_success):
    filter_ = "JSON (*.json);;All files (*)"
    filename = getOpenFileName(
        parent=pywallet_window,
        title=_("Open {} file").format(title),
        filter=filter_,
        config=pywallet_window.config,
    )
    if not filename:
        return
    try:
        ...
        pywallet_window.show_critical(str(e))
    else:
        pywallet_window.show_message(_("Your {} were successfully imported").format(title))
        on_success()


def export_meta_gui(pywallet_window: 'PywalletWindow', title, exporter):
    filter_ = "JSON (*.json);;All files (*)"
    filename = getSaveFileName(
        parent=pywallet_window,
        title=_("Select file to save your {}").format(title),
        filename='pywallet_{}.json'.format(title),
        filter=filter_,
        config=pywallet_window.config,
    )
    if not filename:
    ...
    try:
        ...
        pywallet_window.show_message(_("Your {0} were exported to '{1}'")
