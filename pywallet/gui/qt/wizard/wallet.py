...
from pywallet.bip32 import is_bip32_derivation, BIP32Node, normalize_bip32_derivation, xpub_type
from pywallet.daemon import Daemon
from pywallet.i18n import _
from pywallet.keystore import bip44_derivation, bip39_to_seed, purpose48_derivation, ScriptTypeNotSupported
from pywallet.plugin import run_hook, HardwarePluginLibraryUnavailable
from pywallet.storage import StorageReadWriteError
from pywallet.util import WalletFileException, get_new_wallet_name, UserFacingException, InvalidPassword
from pywallet.util import is_subpath
from pywallet.wallet import wallet_types
from .wizard import QEAbstractWizard, WizardComponent
from pywallet.logging import get_logger, Logger
from pywallet import WalletStorage, mnemonic, keystore
from pywallet.wallet_db import WalletDB
from pywallet.wizard import NewWalletWizard

from pywallet.gui.qt.bip39_recovery_dialog import Bip39RecoveryDialog
from pywallet.gui.qt.password_dialog import PasswordLayout, PW_NEW, MSG_ENTER_PASSWORD, PasswordLayoutForHW
from pywallet.gui.qt.seed_dialog import SeedLayout, MSG_PASSPHRASE_WARN_ISSUE4566, KeysLayout
from pywallet.gui.qt.util import (PasswordLineEdit, char_width_in_lineedit, WWLabel, InfoButton, font_height,
...
if TYPE_CHECKING:
    ...
    from pywallet.daemon import Daemon
    from pywallet.gui.qt import QPywalletApplication

WIF_HELP_TEXT = (_('WIF keys are typed in Pywallet, based on script type.') + '\n\n' +
                 _('A few examples') + ':\n' +
...
class QENewWalletWizard(NewWalletWizard, QEAbstractWizard, MessageBoxMixin):
    ...
    def __init__(self, config: 'SimpleConfig', app: 'QPywalletApplication', plugins: 'Plugins', daemon: Daemon, path, *, start_viewstate=None):
    ...
    def run_split(self, wallet_path, split_data) -> None:
        msg = _(
            "The wallet '{}' contains multiple accounts, which are no longer supported since Pywallet 2.7.\n\n"
...
class WCCreateSeed(WalletWizardComponent):
    ...
    def apply(self):
        if self.slayout:
            ...
            self.wizard_data['seed_variant'] = 'pywallet'
...
class WCHaveSeed(WalletWizardComponent, Logger):
    ...
    def is_seed(self, x):
        ...
        if self.wizard_data['wallet_type'] == 'standard':
            ...
            # multisig?  by default, only accept modern non-2fa pywallet seeds
    ...
    def apply(self):
        ...
        if self.slayout.seed_type == 'pywallet':
        ...
        def scan_task():
            ...
            if not devices:
                ...
                if sys.platform == 'win32':
                    ...
                    msg += _('While this is less than ideal, it might help if you run Pywallet as Administrator.') + '\n'
                else:
