...
import pywallet
from pywallet import version, constants
from pywallet.i18n import _
from pywallet.logging import Logger, get_logger
from pywallet.bip21 import BITCOIN_BIP21_URI_SCHEME, LIGHTNING_URI_SCHEME
from pywallet.base_crash_reporter import BaseCrashReporter, EarlyExceptionsQueue
from pywallet.network import Network
from pywallet.plugin import run_hook
...
if TYPE_CHECKING:
    ...
    from pywallet.wallet import Abstract_Wallet
    from pywallet.daemon import Daemon
    from pywallet.plugin import Plugins
...
class QEAppController(BaseCrashReporter, QObject):
    ...
    def __init__(self, qeapp: 'PywalletQmlApplication', qedaemon: 'QEDaemon', plugins: 'Plugins'):
    ...
    def doNotify(self, wallet_name, message):
        try:
            ...
            icon = (os.path.dirname(os.path.realpath(__file__))
                    + '/../icons/pywallet.png')
            notification.notify('Pywallet', message, app_icon=icon, app_name='Pywallet')
        except ImportError:
...
class PywalletQmlApplication(QGuiApplication):
    ...
    def __init__(self, args, *, config: 'SimpleConfig', daemon: 'Daemon', plugins: 'Plugins'):
        ...
        PywalletQmlApplication._daemon = daemon

        # TODO QT6 order of declaration is important now?
        qmlRegisterType(QEAmount, 'co.caprifin', 1, 0, 'Amount')
        qmlRegisterType(QENewWalletWizard, 'co.caprifin', 1, 0, 'QNewWalletWizard')
        qmlRegisterType(QEServerConnectWizard, 'co.caprifin', 1, 0, 'QServerConnectWizard')
        qmlRegisterType(QEFilterProxyModel, 'co.caprifin', 1, 0, 'FilterProxyModel')
        qmlRegisterType(QSortFilterProxyModel, 'co.caprifin', 1, 0, 'QSortFilterProxyModel')

        qmlRegisterType(QEWallet, 'co.caprifin', 1, 0, 'Wallet')
        qmlRegisterType(QEBitcoin, 'co.caprifin', 1, 0, 'Bitcoin')
        qmlRegisterType(QEQRParser, 'co.caprifin', 1, 0, 'QRParser')
        qmlRegisterType(QEQRScanner, 'co.caprifin', 1, 0, 'QRScanner')
        qmlRegisterType(QEFX, 'co.caprifin', 1, 0, 'FX')
        qmlRegisterType(QETxFinalizer, 'co.caprifin', 1, 0, 'TxFinalizer')
        qmlRegisterType(QEInvoice, 'co.caprifin', 1, 0, 'Invoice')
        qmlRegisterType(QEInvoiceParser, 'co.caprifin', 1, 0, 'InvoiceParser')
        qmlRegisterType(QEAddressDetails, 'co.caprifin', 1, 0, 'AddressDetails')
        qmlRegisterType(QETxDetails, 'co.caprifin', 1, 0, 'TxDetails')
        qmlRegisterType(QEChannelOpener, 'co.caprifin', 1, 0, 'ChannelOpener')
        qmlRegisterType(QELnPaymentDetails, 'co.caprifin', 1, 0, 'LnPaymentDetails')
        qmlRegisterType(QEChannelDetails, 'co.caprifin', 1, 0, 'ChannelDetails')
        qmlRegisterType(QESwapHelper, 'co.caprifin', 1, 0, 'SwapHelper')
        qmlRegisterType(QERequestDetails, 'co.caprifin', 1, 0, 'RequestDetails')
        qmlRegisterType(QETxRbfFeeBumper, 'co.caprifin', 1, 0, 'TxRbfFeeBumper')
        qmlRegisterType(QETxCpfpFeeBumper, 'co.caprifin', 1, 0, 'TxCpfpFeeBumper')
        qmlRegisterType(QETxCanceller, 'co.caprifin', 1, 0, 'TxCanceller')
        qmlRegisterType(QEBip39RecoveryListModel, 'co.caprifin', 1, 0, 'Bip39RecoveryListModel')

        # TODO QT6: these were declared as uncreatable, but that doesn't seem to work for pyqt6
        # qmlRegisterUncreatableType(QEAmount, 'co.caprifin', 1, 0, 'Amount', 'Amount can only be used as property')
        # qmlRegisterUncreatableType(QENewWalletWizard, 'co.caprifin', 1, 0, 'QNewWalletWizard', 'QNewWalletWizard can only be used as property')
        # qmlRegisterUncreatableType(QEServerConnectWizard, 'co.caprifin', 1, 0, 'QServerConnectWizard', 'QServerConnectWizard can only be used as property')
        # qmlRegisterUncreatableType(QEFilterProxyModel, 'co.caprifin', 1, 0, 'FilterProxyModel', 'FilterProxyModel can only be used as property')
        # qmlRegisterUncreatableType(QSortFilterProxyModel, 'co.caprifin', 1, 0, 'QSortFilterProxyModel', 'QSortFilterProxyModel can only be used as property')
        ...
        not_loaded = QFontDatabase.addApplicationFont('pywallet/gui/qml/fonts/PTMono-Regular.ttf') < 0
        not_loaded = QFontDatabase.addApplicationFont('pywallet/gui/qml/fonts/PTMono-Bold.ttf') < 0 and not_loaded
        if not_loaded:
        ...
        self.context.setContextProperty('BUILD', {
            'pywallet_version': version.PYWALLET_VERSION,
            'protocol_version': version.PROTOCOL_VERSION,
            'qt_version': QT_VERSION_STR,
            'pyqt_version': PYQT_VERSION_STR
        })
        self.context.setContextProperty('UI_UNIT_NAME', {
            "FEERATE_SAT_PER_VBYTE": pywallet.util.UI_UNIT_NAME_FEERATE_SAT_PER_VBYTE,
            "FEERATE_SAT_PER_VB":    pywallet.util.UI_UNIT_NAME_FEERATE_SAT_PER_VB,
            "TXSIZE_VBYTES":         pywallet.util.UI_UNIT_NAME_TXSIZE_VBYTES,
            "MEMPOOL_MB":            pywallet.util.UI_UNIT_NAME_MEMPOOL_MB,
        })
