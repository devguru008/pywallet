...
WizardComponent {
    ...
    function apply() {
        ...
        if (cosigner) {
            ...
            // determine script type from pywallet seed type
            // (used to limit script type options for bip39 cosigners)
            if (wizard_data['wallet_type'] == 'multisig' && seed_variant_cb.currentValue == 'pywallet') {
    ...
    function setSeedTypeHelpText() {
        var t = {
            'pywallet': [
                qsTr('Pywallet seeds are the default seed type.'),
                qsTr('If you are restoring from a seed previously created by Pywallet, choose this option')
            ].join(' '),
            'bip39': [
                qsTr('BIP39 seeds can be imported in Pywallet, so that users can access funds locked in other wallets.'),
                '<br/><br/>',
            ...
            'slip39': [
                qsTr('SLIP39 seeds can be imported in Pywallet, so that users can access funds locked in other wallets.'),
                '<br/><br/>',
    ...
    function checkValid() {
        ...
        if (!cosigner || !verifyResult.valid) {
            ...
            if (seed_variant_cb.currentValue == 'pywallet') {
                apply()
    ...
    Flickable {
        ...
        GridLayout {
            ...
            ComboBox {
                ...
                valueRole: 'value'
                model: [
                    { text: qsTr('Pywallet'), value: 'pywallet' },
