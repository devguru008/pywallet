...
try:
    ...
    from pywallet._vendor import pyperclip

import pywallet
from pywallet.gui import BasePywalletGui
from pywallet.bip21 import parse_bip21_URI
from pywallet.util import format_satoshis, format_time
from pywallet.util import EventListener, event_listener
from pywallet.bitcoin import is_address, address_to_script, COIN
...
from pywallet.network import NetworkParameters, TxBroadcastError, BestEffortRequestFailed
...
class PywalletGui(BasePywalletGui, EventListener):
    ...
    def __init__(self, *, config: 'SimpleConfig', daemon: 'Daemon', plugins: 'Plugins'):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'pywallet create'")
            exit()
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy') or out.get('proxy user') or out.get('proxy pass'):
                new_proxy_config = pywallet.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
                if new_proxy_config:
