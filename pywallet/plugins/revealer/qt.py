...
Secret backup plug-in for the pywallet wallet.
...
from pywallet.util import make_dir, InvalidPassword, UserCancelled
from pywallet.gui.qt.util import (read_QIcon, EnterButton, WWLabel, icon_path,
                                  WindowModalDialog, Buttons, CloseButton, OkButton)
from pywallet.gui.qt.qrtextedit import ScanQRTextEdit
...
class Plugin(RevealerPlugin):
    ...
    def __init__(self, parent, config, name):
        ...
        self.base_dir = os.path.join(config.pywallet_path(), 'revealer')
    ...
    def password_dialog(self, msg=None, parent=None):
        from pywallet.gui.qt.password_dialog import PasswordDialog
        ...
        if not calibration_sheet:
            if is_cseed: #its a secret
                ...
                painter.drawImage(((total_distance_h))+11, ((total_distance_h))+11,
                                  QImage(icon_path('pywalletb.png')).scaledToWidth(round(2.1*total_distance_h), Qt.SmoothTransformation))
