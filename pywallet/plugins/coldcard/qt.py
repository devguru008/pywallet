...
from pywallet.gui.qt.util import WindowModalDialog, CloseButton, getOpenFileName, getSaveFileName
...
from pywallet.gui.qt.wizard.wallet import WCScriptAndDerivation, WCHWXPub, WCHWUninitialized, WCHWUnlock
...
class Plugin(ColdcardPlugin, QtPluginBase):
    ...
    @only_hook_if_libraries_available
    @hook
    def wallet_info_buttons(self, main_window: 'PywalletWindow', dialog):
        # user is about to see the "Wallet Information" dialog
        # - add a button if multisig wallet, and a Coldcard is a cosigner.
        assert isinstance(main_window, PywalletWindow), f"{type(main_window)}"
...
class CKCCSettingsDialog(WindowModalDialog):
    ...
    def __init__(self, window: PywalletWindow, plugin, keystore):
        ...
        assert isinstance(window, PywalletWindow), f"{type(window)}"
        self.window = window
