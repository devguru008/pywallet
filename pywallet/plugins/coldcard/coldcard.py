#
# Coldcard Pywallet plugin main code.
#
#
import os
import time
...
from pywallet import bip32
from pywallet.bip32 import BIP32Node, InvalidMasterKeyVersionBytes
from pywallet.i18n import _
from pywallet.plugin import Device, hook, runs_in_hwd_thread
from pywallet.keystore import Hardware_KeyStore, KeyStoreWithMPK
from pywallet.transaction import PartialTransaction
from pywallet.wallet import Standard_Wallet, Multisig_Wallet, Abstract_Wallet
from pywallet.util import bfh, versiontuple, UserFacingException
...
class CKCCClient(HardwareClientBase):
    def __init__(self, plugin, handler, dev_path, *, is_simulator=False):
        ...
        if is_simulator:
            self.dev = PywalletColdcardDevice(dev_path, encrypt=True)
        else:
            # open the real HID device
            hd = hid.device(path=dev_path)
            hd.open_path(dev_path)

            self.dev = PywalletColdcardDevice(dev=hd, encrypt=True)
    ...
    def ping_check(self):
        ...
        req = b'1234 Pywallet Plugin 4321'      # free up to 59 bytes
        try:
...
class Coldcard_KeyStore(Hardware_KeyStore):
    ...
    def _encode_txin_type(txin_type):
        # Map from Pywallet code names to our code numbers.
...
class ColdcardPlugin(HW_PluginBase):
    ...
    def export_ms_wallet(wallet: Multisig_Wallet, fp, name):
        ...
        print('# Exported from Pywallet', file=fp)
