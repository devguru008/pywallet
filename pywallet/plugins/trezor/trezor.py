...
from pywallet.util import bfh, UserCancelled, UserFacingException
from pywallet.bip32 import BIP32Node
from pywallet import descriptor
from pywallet import constants
from pywallet.i18n import _
from pywallet.plugin import Device, runs_in_hwd_thread
from pywallet.transaction import Transaction, PartialTransaction, PartialTxInput, Sighash
from pywallet.keystore import Hardware_KeyStore
from pywallet.logging import get_logger

from pywallet.plugins.hw_wallet import HW_PluginBase
from pywallet.plugins.hw_wallet.plugin import is_any_tx_output_on_change_branch, \
    trezor_validate_op_return_output_and_get_data, LibraryFoundButUnusable, OutdatedHwFirmwareException

if TYPE_CHECKING:
    from pywallet.plugin import DeviceInfo
...
class TrezorKeyStore(Hardware_KeyStore):
    ...
    def add_slip_19_ownership_proofs_to_tx(self, tx: 'PartialTransaction', password) -> None:
        ...
        for txin in tx.inputs():
            ...
            trezor_script_type = self.plugin.get_trezor_input_script_type(desc.to_legacy_pywallet_script_type())
...
class TrezorPlugin(HW_PluginBase):
    ...
    def enumerate(self):
        ...
        # On Windows, the bridge runs as Admin (and Pywallet usually does not),
    ...
    def get_trezor_input_script_type(self, pywallet_txin_type: str):
        if pywallet_txin_type in ('p2wpkh', 'p2wsh'):
            return InputScriptType.SPENDWITNESS
        if pywallet_txin_type in ('p2wpkh-p2sh', 'p2wsh-p2sh'):
            return InputScriptType.SPENDP2SHWITNESS
        if pywallet_txin_type in ('p2pkh',):
            return InputScriptType.SPENDADDRESS
        if pywallet_txin_type in ('p2sh',):
            return InputScriptType.SPENDMULTISIG
        if pywallet_txin_type in ('p2tr',):
            return InputScriptType.SPENDTAPROOT
        raise ValueError('unexpected txin type: {}'.format(pywallet_txin_type))

    def get_trezor_output_script_type(self, pywallet_txin_type: str):
        if pywallet_txin_type in ('p2wpkh', 'p2wsh'):
    ...
    def sign_transaction(self, keystore, tx: PartialTransaction, prev_tx):
        prev_tx = {bfh(txhash): self.pywallet_tx_to_txtype(tx) for txhash, tx in prev_tx.items()}
            ...
            if txin.is_coinbase_input():
                ...
                if for_sig:
                    ...
                    if txin.is_complete() or not txin.is_mine:  # we don't sign
                        ...
                        txinputtype.script_type = self.get_trezor_input_script_type(desc.to_legacy_pywallet_script_type())
    ...
    def tx_outputs(self, tx: PartialTransaction, *, keystore: 'TrezorKeyStore', firmware_version: Sequence[int]):
        ...
        def create_output_by_derivation():
            ...
            assert desc
            script_type = self.get_trezor_output_script_type(desc.to_legacy_pywallet_script_type())
            if multi := desc.get_simple_multisig():
                multisig = self._make_multisig(multi)
            else:
                multisig = None
    ...
    def pywallet_tx_to_txtype(self, tx: Optional[Transaction]):
        t = TransactionType()
        if tx is None:
