#
# BitBox02 Pywallet plugin code.
#

import hid
from typing import TYPE_CHECKING, Dict, Tuple, Optional, List, Any, Callable

from pywallet import bip32, constants
from pywallet.i18n import _
from pywallet.keystore import Hardware_KeyStore
from pywallet.transaction import PartialTransaction, Sighash
from pywallet.wallet import Multisig_Wallet, Deterministic_Wallet
from pywallet.util import UserFacingException
from pywallet.logging import get_logger
from pywallet.plugin import Device, DeviceInfo, runs_in_hwd_thread
from pywallet.simple_config import SimpleConfig
from pywallet.storage import get_derivation_used_for_hw_device_encryption
from pywallet.bitcoin import OnchainOutputType

import pywallet.bitcoin as bitcoin
import pywallet.ecc as ecc
...
class BitBox02Client(HardwareClientBase):
    ...
    def fail_if_not_initialized(self) -> None:
        ...
        if not self.bitbox02_device.device_info()["initialized"]:
            raise BitBox02NotInitialized(
                "Please initialize the BitBox02 using the BitBox app first before using the BitBox02 in pywallet"
            )

    def coin_network_from_pywallet_network(self) -> int:
    ...
    def get_password_for_storage_encryption(self) -> str:
        ...
        xpub = self.bitbox02_device.pywallet_encryption_key(derivation_list)
    ...
    def get_xpub(self, bip32_path: str, xtype: str, *, display: bool = False) -> str:
        ...
        coin_network = self.coin_network_from_pywallet_network()
    ...
    def sign_transaction(
        ...
        for txin in tx.inputs():
            ...
            if tx_script_type is None:
                tx_script_type = desc.to_legacy_pywallet_script_type()
            elif tx_script_type != desc.to_legacy_pywallet_script_type():
