...
from pywallet import bip32, constants, ecc
from pywallet import descriptor
from pywallet.bip32 import BIP32Node, convert_bip32_intpath_to_strpath, normalize_bip32_derivation
from pywallet.bitcoin import EncodeBase58Check, is_b58_address, is_segwit_script_type, var_int
from pywallet.crypto import hash_160
...
from pywallet.transaction import PartialTransaction, Transaction, PartialTxInput
from pywallet.util import bfh, UserFacingException, versiontuple
from pywallet.wallet import Standard_Wallet
    ...
    def _perform_hw1_preflight(self):
        ...
        try:
            ...
            deprecation_warning = (
                ...
                "Future versions of Pywallet will no longer be compatible with it.\n\n"
...
class Ledger_Client_New(Ledger_Client):
    ...
    def sign_transaction(self, keystore: Hardware_KeyStore, tx: PartialTransaction, password: str):
        ...
        try:
            ...
            for input_num, (pywallet_txin, psbt_in) in enumerate(zip(tx.inputs(), psbt.inputs)):
                if pywallet_txin.is_coinbase_input():
                ...
                if utxo is None:
                    continue
                if (desc := pywallet_txin.script_descriptor) is None:
                ...
                if is_wit:
                    ...
                    if pywallet_txin.is_p2sh_segwit():
                        if wit_ver == 0:
                ...
                if multisig is not None:
                    ...
                    # Pywallet uses sortedmulti; we make sure that the array of key information is normalized in a consistent order
