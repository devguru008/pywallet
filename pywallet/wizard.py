...
from pywallet.slip39 import EncryptedSeed
from pywallet.storage import WalletStorage, StorageEncryptionVersion
from pywallet.wallet_db import WalletDB
from pywallet.bip32 import normalize_bip32_derivation, xpub_type
from pywallet import keystore, mnemonic, bitcoin
from pywallet.mnemonic import is_any_2fa_seed_type, can_seed_have_passphrase
    ...
    def create_storage(self, path: str, data: dict):
        ...
        if 'keystore_type' not in data:
            ...
            if data['seed_type'] in ['old', 'standard', 'segwit']:
                self._logger.debug('creating keystore from pywallet seed')
...
class ServerConnectWizard(AbstractWizard):
    ...
    def do_configure_proxy(self, wizard_data: dict):
        ...
        if not self._daemon.network:
            self._logger.debug('not configuring proxy, pywallet config wants offline mode')
            return
