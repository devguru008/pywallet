...
class SimpleConfig(Logger):
    ...
    def __init__(self, options=None, read_user_config_function=None,
        ...
        self.user_config = {}  # for self.get in pywallet_path()
        self.path = self.pywallet_path()
    ...
    def pywallet_path_root(self):
        # Read pywallet_path from command line
        # Otherwise use the user's default data directory.
        path = self.get('pywallet_path') or self.user_dir()
        make_dir(path, allow_symlink=False)
        return path

    def pywallet_path(self):
        path = self.pywallet_path_root()
        if self.get('testnet'):
        ...
        self.logger.info(f"pywallet directory {path}")
        return path
    ...
    def get_wallet_path(self, *, use_gui_last_wallet=False):
        ...
        old_path = os.path.join(self.path, "pywallet.dat")
    ...
    def depth_target_to_fee(self, target: int) -> Optional[int]:
        ...
        #       old PywalletX <1.16 has 1 s/b prec, >=1.16 has 0.1 s/b prec.
...
If this is enabled, other nodes cannot open a channel to you. Channel recovery data is encrypted, so that only your wallet can decrypt it. However, blockchain analysis will be able to tell that the transaction was probably created by Pywallet."""),
    )
...
Warning: setting this to too low will result in lots of payment failures."""),
    ...
    WATCHTOWER_CLIENT_ENABLED = ConfigVar(
        ...
        long_desc=lambda: ' '.join([
            ...
            _("Check our online documentation if you want to configure Pywallet as a watchtower."),
        ]),
    )
...
def read_user_config(path: Optional[str]) -> Dict[str, Any]:
    """Parse and store the user config settings in pywallet.conf into user_config[]."""
    if not path:
        return {}
