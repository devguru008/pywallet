# Copyright (C) 2019 The Pywallet developers
...
# try to make console log lines short... no timestamp, short levelname, no "pywallet."
...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("pywallet."):
    ...
    record.name = record.name.replace("gui.qt.main_window.PywalletWindow", "gui.qt.main_window", 1)
    return record
...
def _delete_old_logs(path, *, num_files_keep: int):
    files = sorted(list(pathlib.Path(path).glob("pywallet_log_*.log")), reverse=True)
...
def _configure_file_logging(log_directory: pathlib.Path, *, num_files_keep: int):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"pywallet_log_{timestamp}_{PID}.log"
...
def _process_verbosity_log_levels(verbosity):
    ...
    for filt in filters:
        ...
        if len(items) == 1:
            level = items[0]
            pywallet_logger.setLevel(level.upper())
        elif len(items) == 2:
...
#       called from run_pywallet (the main script). This is to have this run as early
#       as possible.
# Note: some users might use Pywallet as a python library and not use run_pywallet,
...
if getattr(sys, "_PYWALLET_RUNNING_VIA_RUNPYWALLET", False):
    _inmemory_startup_logs = TruncatingMemoryHandler()
    root_logger.addHandler(_inmemory_startup_logs)

# creates a logger specifically for pywallet library
pywallet_logger = logging.getLogger("pywallet")
pywallet_logger.setLevel(logging.DEBUG)


# --- External API

def get_logger(name: str) -> logging.Logger:
    if name.startswith("pywallet."):
        name = name[9:]
    return pywallet_logger.getChild(name)
...
def configure_logging(config: 'SimpleConfig', *, log_to_file: Optional[bool] = None) -> None:
    ...
    from . import PYWALLET_VERSION
    from .constants import GIT_REPO_URL
    _logger.info(f"Pywallet version: {PYWALLET_VERSION} - https://wallet.caprifin.co - {GIT_REPO_URL}")
