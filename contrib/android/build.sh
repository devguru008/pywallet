...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t pywallet-android-builder-img \
...
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    ...
    FRESH_CLONE=${FRESH_CLONE:-"/tmp/pywallet_build/android/fresh_clone/pywallet"}
...
docker run -it --rm \
    --name pywallet-android-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/home/user/wspace/pywallet \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT"/.buildozer/.gradle:/home/user/.gradle \
    $DOCKER_RUN_FLAGS \
    --workdir /home/user/wspace/pywallet \
    pywallet-android-builder-img \
