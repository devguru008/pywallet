...
if ! which ${genisoimage} > /dev/null 2>&1; then
    mkdir -p /tmp/pywallet-macos
    cd /tmp/pywallet-macos
...
rm -rf /tmp/pywallet-macos/image > /dev/null 2>&1
mkdir /tmp/pywallet-macos/image/
cp -r $1 /tmp/pywallet-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Pywallet_uncompressed.dmg \
    /tmp/pywallet-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Pywallet_uncompressed.dmg pywallet-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Pywallet_uncompressed.dmg

echo "Done."
sha256sum pywallet-$VERSION.dmg
