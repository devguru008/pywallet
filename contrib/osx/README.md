...
This guide explains how to build Pywallet binaries for macOS systems.
...
- The unix username should be `vagrant`, and `pywallet` should be cloned directly
  to the user's home dir: `/Users/vagrant/pywallet`.
...
- Installing extraneous brew packages can result in build differences.
  ...
  So having a dedicated "pywallet binary builder macOS VM" is recommended.
- Make sure that you are building from a fresh clone of pywallet
...
This creates both a folder named Pywallet.app and the .dmg file (both unsigned).
...
also run the `sign_osx.sh` script:
    ...
    CODESIGN_CERT="Developer ID Application: Pywallet Technologies GmbH (L6P37P7P56)" \
...
2. Use the provided `compare_dmg` script to compare the binary you built with
   the official release binary.
    ```
    $ ./contrib/osx/compare_dmg dist/electrum-*.dmg pywallet_dmg_official_release.dmg
    ```
