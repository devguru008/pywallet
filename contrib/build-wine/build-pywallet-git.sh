...
pushd $WINEPREFIX/drive_c/pywallet
...
info "Pip installing Pywallet. This might take a long time if the project folder is large."
$WINE_PYTHON -m pip install --no-build-isolation --no-dependencies --no-warn-script-location .
# pyinstaller needs to be able to "import pywallet", for which we need libsecp256k1:
# (or could try "pip install -e" instead)
cp pywallet/libsecp256k1-*.dll "$WINEPREFIX/drive_c/python3/Lib/site-packages/pywallet/"
popd
...
PYWALLET_CMDLINE_NAME="$NAME_ROOT-$VERSION" wine "$WINE_PYHOME/scripts/pyinstaller.exe" --noconfirm --clean deterministic.spec
...
# $VERSION could be passed to the pywallet.nsi script, but this would require some rewriting in the script itself.
makensis -DPRODUCT_VERSION=$VERSION pywallet.nsi

cd dist
mv pywallet-setup.exe $NAME_ROOT-$VERSION-setup.exe
cd ..
