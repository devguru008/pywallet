...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t pywallet-wine-builder-img \
    "$CONTRIB_WINE"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/pywallet_build/windows/fresh_clone/pywallet"
...
docker run -it \
    --name pywallet-wine-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/wine64/drive_c/pywallet \
    --rm \
    --workdir /opt/wine64/drive_c/pywallet/contrib/build-wine \
    pywallet-wine-builder-img \
    ./make_win.sh
