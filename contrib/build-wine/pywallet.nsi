...
;Variables
  ...
  !define PRODUCT_NAME "Pywallet"
  !define PRODUCT_WEB_SITE "https://github.com/devguru008/pywallet"
  !define PRODUCT_PUBLISHER "Caprifin Co.LTD"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/pywallet-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "..\..\pywallet\gui\icons\pywallet.ico"
...
Section
  ...
  File /r "dist\pywallet\*.*"
  File "..\..\pywallet\gui\icons\pywallet.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\pywallet-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\pywallet-${PRODUCT_VERSION}.exe" "" "$INSTDIR\pywallet-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\pywallet-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\pywallet-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: and lightning: URIs to Pywallet
  WriteRegStr HKCU "Software\Classes\bitcoin" "" "URL:bitcoin Protocol"
  WriteRegStr HKCU "Software\Classes\bitcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcoin" "DefaultIcon" "$\"$INSTDIR\pywallet.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoin\shell\open\command" "" "$\"$INSTDIR\pywallet-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "Software\Classes\lightning" "DefaultIcon" "$\"$INSTDIR\pywallet.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\pywallet-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\pywallet.ico"
