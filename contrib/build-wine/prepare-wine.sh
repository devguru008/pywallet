...
KEYRING_PYTHON_DEV="keyring-pywallet-build-python-dev.gpg"
...
cp "$DLL_TARGET_DIR"/libsecp256k1-*.dll $WINEPREFIX/drive_c/pywallet/pywallet/ || fail "Could not copy libsecp to its destination"
cp "$DLL_TARGET_DIR/libzbar-0.dll" $WINEPREFIX/drive_c/pywallet/pywallet/ || fail "Could not copy libzbar to its destination"
cp "$DLL_TARGET_DIR/libusb-1.0.dll" $WINEPREFIX/drive_c/pywallet/pywallet/ || fail "Could not copy libusb to its destination"
...
(
    ...
    cd "$WINEPREFIX/drive_c/pywallet"
