...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t pywallet-appimage-builder-img \
    "$CONTRIB_APPIMAGE"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/pywallet_build/appimage/fresh_clone/pywallet"
...
docker run -it \
    --name pywallet-appimage-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/pywallet \
    --rm \
    --workdir /opt/pywallet/contrib/build-linux/appimage \
    pywallet-appimage-builder-img \
    ./make_appimage.sh
